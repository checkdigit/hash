...
import { HASH_NAMESPACE } from './hash.ts';
import uuidV5 from './uuid-v5.ts';

describe('uuidV5', () => {
  it('uuidV5 result matches v5(https://github.com/uuidjs/uuid) result', () => {
    assert.equal(
      uuidV5('Hello World!', HASH_NAMESPACE),
      v5('Hello World!', HASH_NAMESPACE),
    );
    assert.equal(
    ...
    assert.equal(
      uuidV5('Råma Ghantå', HASH_NAMESPACE),
      v5('Råma Ghantå', HASH_NAMESPACE),
    );
    assert.equal(
      uuidV5(
        // eslint-disable-next-line no-secrets/no-secrets
        '/hYSjdEGQI0DaVHCR0vZq0KeF5JLys3jSNvU9EzBMKKlSsC97xFd3wr+nrhzHhp1',
        HASH_NAMESPACE,
      ),
      v5(
        // eslint-disable-next-line no-secrets/no-secrets
    ...
    assert.equal(
      ...
      uuidV5('5111111111111118', HASH_NAMESPACE),
      // eslint-disable-next-line @checkdigit/no-card-numbers
      v5('5111111111111118', HASH_NAMESPACE),
    );
    assert.equal(
      uuidV5('https://checkdigit.com', HASH_NAMESPACE),
      v5('https://checkdigit.com', HASH_NAMESPACE),
    );
  });

  it('v5 - from https://github.com/uuidjs/uuid testing', () => {
    assert.equal(
      uuidV5('hello.example.com', v5.DNS),
      'fdda765f-fc57-5604-a269-52a7df8164ec',
    );

    assert.equal(
      uuidV5('http://example.com/hello', v5.URL),
      '3bbcee75-cecc-5b56-8031-b6641c1ed1f1',
    );

    assert.equal(
      uuidV5('hello', '0f5abcd1-c194-47f3-905b-2df7263a084b'),
  ...
  it('v5 namespace.toUpperCase - from https://github.com/uuidjs/uuid testing', () => {
    assert.equal(
      uuidV5('hello.example.com', v5.DNS.toUpperCase()),
      'fdda765f-fc57-5604-a269-52a7df8164ec',
    );
